...
ЛОКАЛЬНЫЕ переменные: -определены в блоке или в функции
	...
	-static -глобальное время жизни в методе или в функции (записываются в оперативную)
		сохраняет предыдущие результаты, 
...
ФУНКЦИИ: возвращ. тип любой кроме массива и функции, но может быть указатель на массив или ф-цию
	-static
СТЕК
	аргументы, адрес возврата(связан return), адреса вложенных кадров(адреса на локальные переменные вызывающего метода), локальные переменные, сохраненные регистры процессора
(во всех функциях return подставляется компилятором при void, чтобы выйти из функции)
...
К указателю можно присваивать указатель или ссылку на ячейку памяти

	char *pc=(char*)0xB800 0000;
	int *N = NULL; int *R = 0;
Разность указателей -это разность их значений,деленная на размер типа в байтах ((3-4)/sizeof(int)-если int)
...
int x = 4; -глобальная переменная
::x;-обращение к глобальной переменной из любого места
